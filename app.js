...
const connection = mysql.createConnection({
    ...
    user: 'b456dabbdb9103', 
    password: '6e8e8360', 
    database: 'heroku_58996c17fc4edb2'
});
// incidents database
const connection2 = mysql.createConnection({
    ...
    user: 'b4f8a5d63fa5ab',
    password: '5b8c84c9',
    database: 'heroku_5450138b9583394'
});
...
app.get('/codes', (req, res) => {
    ...
    // Get data
    databaseSelect(query, params, connection)
        .then((data) => {
...
app.put('/new-incident', (req, res) => {
    ...
    let metaDataQuery = `SELECT  column_name, data_type FROM information_schema.columns WHERE table_name = 'incidents';`;
    
    databaseSelect(selectQuery, [case_No], connection2)
        .then((data) => {
            ...
            return databaseSelect(metaDataQuery, [], connection2)
        })
        .then((data) => {
            ...
            for (let x in data) {
                ...
                if (data[x].data_type == "varchar") {
                    neededInputsTypes.push("string");
                } else if (data[x].data_type == "int") {
                    neededInputsTypes.push("number");
                } else {
                    neededInputsTypes.push(data[x].data_type);
                }
                neededInputs.push(data[x].column_name); // Column names from database
            }
            ...
            console.log(neededInputs, neededInputsTypes)
            ...
            for (let key in neededInputs) {
                ...
                console.log(insertQueryParams[key], neededInputsTypes[key])
...
app.delete('/remove-incident', (req, res) => {
    ...
    databaseSelect(selectQuery, [incident_num], connection2)
        .then((data) => {
            if (data.length === 0) {
...
function databaseSelect(query, params, connection) {
...
function databaseRun(query, params) {
    return new Promise((resolve, reject) => {
        connection2.query(query, params, (err, rows) => {
            if (err) {
                reject(err);
            }
            else {
...
// // Render HOme Page
// app.get('/codes', (req, res) => {
//     connection.query('SELECT * FROM codes', (error, rows) => {
//         if (error) {
//             throw error; 
//         } 
//         if (!error) {
...
// app.get('/incidents', (req, res) => {
//     connection2.query('SELECT * FROM incidents limit 10', (error, rows) => {
//         if (error) {
//             throw error; 
//         } 
//         if (!error) {
